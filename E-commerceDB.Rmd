...
con <- dbConnect(RSQLite::SQLite(), "ecommerce.db")
=======
>>>>>>> 36eef8688a3764f49966d4c0ddb939bbd4070f55
...
<<<<<<< HEAD
>>>>>>> fefe05e713bcfd56b2ea8f240dff87db2e7d66e9
=======
```{r}
...
Customers <- Customers %>% 
  mutate(Cust_Name = paste(Cust_First_Name,Cust_Last_Name))
Customers <- Customers %>% select(-Cust_First_Name, -Cust_Last_Name)
...
# table_exists <- dbExistsTable(con, "Reviews")
# 
# if (table_exists) {
#   # If the table exists and want to append the data 
#   dbWriteTable(conn = con, name = "Reviews", value = Reviews, append = TRUE, row.names = FALSE)
# } else {
#   # If table doesnt exist or if want to overwrite the existing table 
#   dbWriteTable(conn = con, name = "Reviews", value = Reviews, overwrite = TRUE, row.names = FALSE)
# }
