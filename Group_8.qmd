...
<p> First Normal Form (1NF) requires a table to have atomic values, unique rows through a primary key, and no duplicate columns.Thus , based on this definition the tables have been checked and considered to have atomic values with no duplicated columns. </p>

<p> Next, the team  considered the Second Normal Form (2NF) which can be achieved when a table is in 1NF and all non-key attributes are fully functional and dependent on the primary key, eliminating partial dependency on any part of a composite key. </p>

<p> As a last normal form to be considered and for a table to be in third normal form (3NF), it should not have any transitive dependencies, and all non-prime attributes are fully functionally dependent on the primary key. The team perfoemed the required checks until all requirements of (3NF) are met as clarified in details in the following sections. </p> 
...
Product_ID , Order_ID → Order_Item 

Product_ID and Order_ID combined are representing composite key, and Order_Item is functionally dependent on Product_ID and Order_ID. There are no transitive dependencies here.
...
Product_ID → Product_Brand \newline Product_ID → Product_Name	\newline Product_ID → Product_Model \newline Product_ID → Product_Price	\newline Product_ID → Product_Availability \newline Product_ID → Supplier_ID \newline Product_ID → Discount_Code \newline Product_ID → Category_ID

The table also contains foreign keys, namely Supplier_ID, Discount_Code, and Category_ID. These foreign keys are merely referencing the primary key of their respective tables and are not functionally dependent on any other attribute within this table.

Examining the non key attributes' functional dependencies, Product_ID emerges as the primary key, in which Product_Name, Product_Price and Product_Availability appears to be functionally dependent on it.
...
<p> ![data validate steps](Images/data_validate.PNG){width=250px height=300px} </p>
...
<p>Second phase was the automation process utilising workflow that trigger on specific events like push request to perform data updates, validation, and analysis. This will pick up any new data added to the data base and subsequently perform the required validation and analysis. 

<p> **Figures 9, 10** shows the record count after 1st run of workflow (49) and then after second run of workflow (150).</p>

<p> ![The histogram of customers data for 50 records](Images/Workflow1.png "Figure 9 - The histogram of customers data for 50 records"){width=450px height=450px} </p>

<p> ![The histogram after workflow run and inserted additional 100 records](Images/Workflow2.png "Figure 10 -The histogram after workflow run and inserted additional 100 records"){width=450px height=450px} </P>

**Figure 11** illustrate console output showing the record count after data update using the workflow.

<p> ![Record count after data update using workflow](Images/Console_Workflow.png "Figure 11 - Console Output"){width=450px height=450px} </p>

<p> In **Figure 12** the snippet is Timestamps that showed as an evidence of a working workflow </p>

<p> ![Workflow Timestamps](Images/WorkflowTimestamps.png "Figure 12 - Workflow Timestamps"){width=450px height=450px} </p>


# Part 4: Data Analysis and Reporting with Quarto in R

<p> ![Distribution of Product Ratings by Category](Images/Product_Rating_By_Category.jpeg "Figure 13 - Distribution of Product Ratings by Category"){width=450px height=450px} </p>
...
<p> The graph in **Figure 13** shows the distribution of product ratings from 0 to 5 coloured by category name. </p>

<p> ![Top 5 Products by Revenue](Images/Top_5_Products_Revenue.png "Figure 14 - Top 5 Products by Revenue"){width=450px height=450px} </p>

<p> **Figure 14** reflecting Top 5 Products Sorted by total Revenue.</p>
...
<p> ![Distribution of Customers by Phone Country Code](Images/Cust_Phone_Country_Code_Distribution.png "Figure 15- Distribution of Customers by Phone Country Code"){width=400px height=400px} </p>

<p> The graph in **Figure 15**  shows the distribution of customers by phone country codes.</p>
 

<p> **Figure 16** displays the order status count of shipped, processing, delivered and cancelled orders. </p>

<p> ![Order Status Count](Images/Order_Status_Distribution.png "Figure 16- Order Status Count"){width=400px height=400px} </p>
